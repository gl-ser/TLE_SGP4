...
TLE_SGP4::TLE_SGP4()
{
  //Установка десятичного разделителя decimal_point в значение "."
  // От этого зависит работа системной функции strtod из stdlib.h
...
bool TLE_SGP4::ReadTLE(std::string Str1, std::string Str2)
{
  bool res;
  double xpdotp = 1440.0/(2.0*M_PI);

  if ((Str1.length() >= 69) && (Str2.length() >= 69) && (Str1[0] == '1') && (Str2[0] == '2'))
  {
    rec.whichconst=2;  //wgs72
    parseEpoch(&const_cast<char*>(Str1.c_str())[18]);  //Строка 1 Секции 7, 8 - год эпохи, время эпохи
    rec.elnum = static_cast<long>(gd(const_cast<char*>(Str1.c_str()), 64, 68));   //Строка 1 Секция 13 - номер элемента
    rec.revnum = static_cast<long>(gd(const_cast<char*>(Str2.c_str()), 63, 68));  //Строка 2 Секция 9 - номер витка на момент эпохи
    strncpy(rec.satid, &const_cast<char*>(Str1.c_str())[2], 5);                   //Строка 1 Секция 2 - номер спутника в базе NORAD
    rec.satid[5] = '\0';
    rec.classification = Str1[7];                      //Строка 1 Секция 3 - классификация
    rec.bstar = gdi(const_cast<char*>(Str1.c_str()), 54, 59);
    if (Str1[53] == '-')
    {
      rec.bstar *= -1.0;
    }
    rec.bstar *= pow(10.0, gd(const_cast<char*>(Str1.c_str()), 59, 61));   //Строка 1 Секция 11 - коэффициент торможения
    rec.inclo = gd(const_cast<char*>(Str2.c_str()), 8, 16)*(M_PI/180.0);   //Строка 2 Секция 3 - наклонение в градусах
    rec.nodeo = gd(const_cast<char*>(Str2.c_str()), 17, 25)*(M_PI/180.0);  //Строка 2 Секция 4 - долгота восходящего узла в градусах
    rec.argpo = gd(const_cast<char*>(Str2.c_str()), 34, 42)*(M_PI/180.0);  //Строка 2 Секция 6 - аргумент перицентра в градусах
    rec.mo = gd(const_cast<char*>(Str2.c_str()), 43, 51)*(M_PI/180.0);     //Строка 2 Секция 7 - средняя анамалия в градусах
    rec.ecco = gdi(const_cast<char*>(Str2.c_str()), 26, 33);               //Строка 2 Секция 5 - эксцентриситет
    rec.no_kozai = gd(const_cast<char*>(Str2.c_str()), 52, 63)/xpdotp;     //Строка 2 Секция 8 - частота обращения (витков в сутки)
    rec.ndot = gdi(const_cast<char*>(Str1.c_str()), 35, 44);
    if(Str1[33] == '-') rec.ndot *= -1.0;
    rec.ndot = rec.ndot / (xpdotp*1440.0);             //Строка 1 Секция 9 - первая производная от среднего движения, деленная на 2
    rec.nddot = gdi(const_cast<char*>(Str1.c_str()), 45, 50);
    if(Str1[44] == '-') rec.nddot *= -1.0;
    rec.nddot *= pow(10.0, gd(const_cast<char*>(Str1.c_str()), 50, 52));
    rec.nddot = rec.nddot / (xpdotp*1440.0*1440.0);    //Строка 1 Секция 10 - вторая производная от среднего движения, деленная на 6
...
int TLE_SGP4::GetRV(double MinutesAfterEpochTLE, double R[], double V[])
{
  rec.error = 0;
  sgp4(&rec, MinutesAfterEpochTLE, R, V);
...
void TLE_SGP4::VixTZ(int Year, int Month, int Day, int Hour, int Min, int Sec, double DSec, double R[], double V[])
{
  double Julian, JulianFrac, TimeCur, TimeNU;
  ...
  if (TLE_Time.Year != 0)
  {
    jday(Year, Month, Day, Hour, Min, static_cast<double>(Sec)+DSec, &Julian, &JulianFrac);
    TimeCur = Julian + JulianFrac - 0.125;

    jday(TLE_Time.Year, TLE_Time.Month, TLE_Time.Day, TLE_Time.Hour, TLE_Time.Min,
         static_cast<double>(TLE_Time.Sec)+TLE_Time.DSec, &Julian, &JulianFrac);
    TimeNU = Julian + JulianFrac;

    if (TimeCur >= TimeNU)
    {
      rec.error = 0;
      sgp4(&rec, (TimeCur-TimeNU)*1440.0, R, V);
      if (rec.error == 0)
      {
        R[0] = R[0]*1000.0;
        R[1] = R[1]*1000.0;
...
void TLE_SGP4::GetCurrentTime(int *Year, int *Month, int *Day, int *Hour, int *Min, int *Sec)
{
  time_t now = time(nullptr);  //Текущее системное время
  tm *ltm = localtime(&now);   //Разобрать системное время
...
----------------------------------------------------------------------------*/
void TLE_SGP4::dpper(double e3, double ee2, double peo, double pgho, double pho, double pinco, double plo, double se2, double se3,
                     double sgh2, double sgh3, double sgh4, double sh2, double sh3, double si2, double si3, double sl2, double sl3,
                     double sl4, double t, double xgh2, double xgh3, double xgh4, double xh2, double xh3, double xi2, double xi3,
                     double xl2, double xl3, double xl4, double zmol, double zmos, char init, TLE_SGP4::ElsetRec *rec, char opsmode)
{
  //--------------------- local variables ------------------------
...
*    vallado, crawford, hujsak, kelso  2006
----------------------------------------------------------------------------*/
void TLE_SGP4::dscom(double epoch, double ep, double argpp, double tc, double inclp, double nodep, double np, TLE_SGP4::ElsetRec *rec)
{
  //-------------------------- constants -------------------------
  ...
  xnodce = fmod(4.5236020 - 9.2422029e-4 * rec->day, 2.0*M_PI);
  stem = sin(xnodce);
  ctem = cos(xnodce);
  ...
  rec->zmol = fmod(4.7199672 + 0.22997150  * rec->day - rec->gam, 2.0*M_PI);
  rec->zmos = fmod(6.2565837 + 0.017201977 * rec->day, 2.0*M_PI);
...
*    vallado, crawford, hujsak, kelso  2006
----------------------------------------------------------------------------*/
void TLE_SGP4::dsinit(double tc, double xpidot, TLE_SGP4::ElsetRec *rec)
{
  //--------------------- local variables ------------------------
  ...
  rec->dndt = 0.0;
  theta = fmod(rec->gsto + tc * rptim, 2.0*M_PI);
  ...
  if (rec->irez != 0)
  {
    ...
    if (rec->irez == 2)
    {
      ...
      rec->xlamo = fmod(rec->mo + rec->nodeo + rec->nodeo - theta - theta, 2.0*M_PI);
    ...
    if (rec->irez == 1)
    {
      ...
      rec->xlamo = fmod(rec->mo + rec->nodeo + rec->argpo - theta, 2.0*M_PI);
...
*    vallado, crawford, hujsak, kelso  2006
----------------------------------------------------------------------------*/
void TLE_SGP4::dspace(double tc, TLE_SGP4::ElsetRec *rec)
{
  int iretn;
...
*    vallado, crawford, hujsak, kelso  2006
----------------------------------------------------------------------------*/
void TLE_SGP4::initl(double epoch, TLE_SGP4::ElsetRec *rec)
{
  //--------------------- local variables ------------------------
  ...
  double gsto1 = fmod(thgr70 + c1*ds70 + c1p2p*tfrac + ts70*ts70*fk5r, 2.0*M_PI);

  if (gsto1 < 0.0)
    gsto1 = gsto1 + 2.0*M_PI;
...
*    vallado, crawford, hujsak, kelso  2006
----------------------------------------------------------------------------*/
bool TLE_SGP4::sgp4init(char opsmode, TLE_SGP4::ElsetRec *satrec)
{
  //--------------------- local variables ------------------------
  double cc1sq, cc2, cc3, coef, coef1, cosio4,
...
*    vallado, crawford, hujsak, kelso  2006
----------------------------------------------------------------------------*/
bool TLE_SGP4::sgp4(TLE_SGP4::ElsetRec *satrec, double tsince, double *r, double *v)
{
  double axnl, aynl, betal, cnod, cos2u, coseo1, cosi, cosip, cosisq, cossu, cosu,
  ...
  satrec->nodem = fmod(satrec->nodem, 2.0*M_PI);
  satrec->argpm = fmod(satrec->argpm, 2.0*M_PI);
  xlm = fmod(xlm, 2.0*M_PI);
  satrec->mm = fmod(xlm - satrec->argpm - satrec->nodem, 2.0*M_PI);
  ...
  u = fmod(xl - satrec->nodep, 2.0*M_PI);
  eo1 = u;
  tem5 = 9999.9;
  ktr = 1;
  sineo1 = 0;
...
*    vallado, crawford, hujsak, kelso  2006
--------------------------------------------------------------------------- */
void TLE_SGP4::getgravconst(int whichconst, TLE_SGP4::ElsetRec *rec)
{
...
* --------------------------------------------------------------------------- */
double TLE_SGP4::gstime(double jdut1)
{
  double temp, tut1;
  ...
  temp = fmod(temp * (M_PI/180.0) / 240.0, 2.0*M_PI);  //360/86400 = 1/240, to deg, to rad
...
* --------------------------------------------------------------------------- */
void TLE_SGP4::jday(int year, int mon, int day, int hr, int minute, double sec, double *jd, double *jdfrac)
{
  *jd = 367.0 * year - floor((7 * (year + floor((mon + 9) / 12.0))) * 0.25) + floor(275 * mon / 9.0) +
...
double TLE_SGP4::fmod(double numer, double denom)
{
  long tquot = static_cast<long>(floor(numer/denom));
  return numer-tquot*denom;
}


long TLE_SGP4::parseEpoch(char *str)
{
  char tmp[16];
  strncpy(tmp, str, 14);
  tmp[15]=0;
  ...
  tmp2[11]=0;
  double dfrac = strtod(tmp2, nullptr);
  rec.epochdays = doy;
  rec.epochdays += dfrac;
  ...
  TLE_Time.Hour = hr;
  TLE_Time.Min = mn;
  TLE_Time.Sec = static_cast<int>(sec);
  TLE_Time.DSec = sec - static_cast<double>(TLE_Time.Sec);
  ...
  long epoch = static_cast<long>(diff2);
  epoch+=static_cast<long>(diff);
  return epoch;
}


double TLE_SGP4::gd(char *str, int ind1, int ind2)
{
  double num = 0;
  char tmp[50];
  int cnt = ind2-ind1;
  strncpy(tmp, &str[ind1], static_cast<unsigned int>(cnt));
  tmp[cnt] = 0;
  num = strtod(tmp, nullptr);
  return num;
}


double TLE_SGP4::gdi(char *str, int ind1, int ind2)
{
  double num = 0;
  char tmp[52];
  tmp[0] = '0';
  tmp[1] = '.';
  int cnt = ind2-ind1;
  strncpy(&tmp[2], &str[ind1], static_cast<unsigned int>(cnt));
  tmp[2+cnt] = 0;
  num = strtod(tmp, nullptr);
  return num;
}
